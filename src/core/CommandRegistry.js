...
class CommandRegistry {
  ...
  load() {
    ...
    this.commands.clear(); this.aliases.clear();
    const files = fs.readdirSync(this.commandsDir).filter(file => file.endsWith('.js')).sort();
    for (const file of files) {
      const fullPath = path.join(this.commandsDir, file); delete require.cache[require.resolve(fullPath)];
      try {
        const command = require(fullPath);
        if (!command || typeof command !== 'object' || !command.name || typeof command.execute !== 'function') { this.logger.warn(`Skipping invalid command module: ${file}`); continue; }
        ...
        normalized.aliases = [...new Set((Array.isArray(normalized.aliases) ? normalized.aliases : [normalized.aliases]).filter(v => v !== null && v !== undefined).map(String).map(v => v.trim().toLowerCase()).filter(Boolean))];
        ...
        if (!normalized.name || /\s/.test(normalized.name)) { this.logger.warn(`Skipping command with invalid name in ${file}.`); continue; }
        if (this.commands.has(normalized.name) || this.aliases.has(normalized.name)) { this.logger.warn(`Skipping duplicate command name: ${normalized.name}`); continue; }
        const usableAliases = [];
        for (const alias of normalized.aliases) {
          if (alias === normalized.name || this.commands.has(alias) || this.aliases.has(alias)) { this.logger.warn(`Ignoring conflicting alias "${alias}" on command ${normalized.name}.`); continue; }
          usableAliases.push(alias);
        }
        normalized.aliases = usableAliases; this.commands.set(normalized.name, normalized);
    ...
    this.logger.info(`Loaded ${this.commands.size} command(s).`); return this;
  }

  get(name) { const key = String(name || '').trim().toLowerCase(); return this.commands.get(key) || this.commands.get(this.aliases.get(key)); }
  list() { return [...this.commands.values()]; }
  _cooldownKey(command, userID, threadID) { return `${command.name}:${threadID || 'global'}:${userID || 'unknown'}`; }
  _remainingCooldown(command, userID, threadID) { const seconds = Number(command.cooldown || 0); if (seconds <= 0) return 0; const key = this._cooldownKey(command, userID, threadID); const expiresAt = this.cooldowns.get(key) || 0; if (expiresAt <= Date.now()) { this.cooldowns.delete(key); return 0; } return expiresAt - Date.now(); }
  ...
  async execute(api, message) {
    ...
    const input = body.slice(prefix.length).trim(); if (!input) return false;
    ...
    ctx.group = group || this.services.groups?.get?.(message.threadID); ctx.user = await this.db.ensureUser?.(message.senderID, message.senderName || '');
    ...
    const run = async () => {
      try {
        ...
        this.state?.incrementStat('commandsExecuted'); return true;
      } catch (error) {
        this.state?.incrementStat('errorsEncountered'); this.logger.error(`Command ${command.name} failed:`, error);
    ...
    try { return this.services.performance?.run ? await this.services.performance.run(run) : await run(); }
    catch (error) { this.logger.warn(`Command ${command.name} was rejected by the performance limiter: ${error.message}`); const text = this.services.formatter?.error('The bot is busy right now. Please try again shortly.') || 'The bot is busy right now. Please try again shortly.'; try { await api.sendMessage(text, message.threadID); } catch (sendError) { this.logger.error('Failed to send overload response:', sendError); } return true; }
  }
