...
class ModerationManager {
  constructor({ db, groups, permissions, state, logger } = {}) {
    Object.assign(this, { db, groups, permissions, state, logger });
    this.windows = new Map();
  }

  _group(threadID) {
    return this.groups?.get(threadID) || this.db?.getGroup(threadID);
  }

  _isExempt(userID, threadID) {
    return this.permissions?.hasLevel(userID, threadID, 1) || false;
  }
  ...
  async inspect(event) {
    ...
    if (group.antiLink && URL_RE.test(String(event.body))) {
      return this._recordViolation(event, 'link', 'Links are not allowed in this group.');
    }
    ...
    if (group.antiSpam) {
      ...
      const now = Date.now();
      const recent = (this.windows.get(key) || []).filter(timestamp => now - timestamp < windowMs);
      recent.push(now);
      this.windows.set(key, recent);
      if (recent.length >= limit) {
        this.windows.set(key, []);
        return this._recordViolation(event, 'spam', `Slow down. Anti-spam detected ${limit} messages in ${Math.round(windowMs / 1000)}s.`);
      }
    }
  ...
  warnings(userID) {
    return Number(this.db.getUser(userID)?.warnings || 0);
  }

  async clearWarnings(userID, threadID, actorID) {
    const user = await this.db.ensureUser(userID);
    const previous = Number(user.warnings || 0);
    user.warnings = 0;
    this.db.data.history.push({
      type: 'moderation:clearwarnings',
      userID: String(userID),
      threadID: String(threadID),
      actorID: String(actorID || ''),
      previous,
